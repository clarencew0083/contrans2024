...
class contrans:
    ...
    def connect_to_mongo(self, from_scratch = False, host='localhost'):
        myclient = pymongo.MongoClient(f'mongodb://{self.MONGO_INITDB_ROOT_USERNAME}:{self.MONGO_INITDB_ROOT_PASSWORD}@l{host}:27017/')
    ...
    def upload_many_members_to_mongo(self, mongo_bills, members):
        i = 1
        for m in members:
            status = f'Now uploading bills from {m} members to MongoDB: legislator {i} of {len(members)}'
            print(status)
            try:
                self.upload_one_member_to_mongo(mongo_bills, m)
            except:
                print(f"Failed to upload legislator {m}")
            i += 1
...
### Analyses
    ...
    def plot_ideology(self, bioguide_id):
            server, engine = self.connect_to_postgres(self.POSTGRES_PASSWORD, create_contrans=False)
            myquery = '''
